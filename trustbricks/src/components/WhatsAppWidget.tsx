          ...
          <AnimatePresence>
            {isOpen && (
              <motion.div
                ...
                className="w-[90vw] sm:w-[400px] h-[550px] rounded-3xl bg-white border border-slate-200/80 shadow-[0_20px_50px_rgba(13,31,60,0.15)] flex flex-col overflow-hidden relative"
              >
                {/* Custom Gradient Background glow inside chat */}
                <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-b from-[var(--color-amber-500)]/10 to-transparent pointer-events-none" />

                {/* Header */}
                <div className="bg-[#050B14] p-4 text-white flex items-center justify-between z-10">
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center border border-white/10">
                        <Bot className="w-5 h-5 text-[var(--color-amber-500)]" />
                      </div>
                      <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-500 border-2 border-[#050B14] rounded-full animate-pulse" />
                    </div>
                    <div>
                      <h4 className="font-bold text-sm tracking-tight flex items-center gap-1.5">
                        Ada <span className="text-[10px] bg-[var(--color-amber-500)]/20 text-[var(--color-amber-500)] px-2 py-0.5 rounded-full font-black uppercase">AI Advisor</span>
                      </h4>
                      <p className="text-[11px] text-slate-400">TrustBricks Mortgage Assistant</p>
                    </div>
                  </div>
                  <button 
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
                    ...
                    >
                      <div
                        className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm shadow-sm ${
                          msg.sender === "user"
                            ? "bg-[#050B14] text-white rounded-tr-none"
                  ...
                  {isLoading && (
                    <div className="flex justify-start">
                      <div className="bg-white border border-slate-200/60 rounded-2xl rounded-tl-none px-4 py-3 text-sm shadow-sm flex items-center gap-1.5">
                ...
                {/* Quick replies */}
                <div className="p-2 bg-slate-50 border-t border-slate-100 flex gap-2 overflow-x-auto scrollbar-none z-10">
                  {quickReplies.map((reply, i) => (
                    <button
                      key={i}
                      onClick={() => handleSendMessage(reply)}
                      className="whitespace-nowrap px-3 py-1.5 bg-white border border-slate-200 rounded-full text-xs font-bold text-slate-600 hover:border-[var(--color-amber-500)] hover:text-[var(--color-amber-500)] transition-all shadow-sm"
                    >
                      {reply}
                    </button>
                  ))}
                ...
                {/* Bottom Input Area */}
                <div className="p-3 bg-white border-t border-slate-100 flex flex-col gap-2.5 z-10">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-[var(--color-amber-500)] text-sm text-slate-800 bg-slate-50/50"
                    />
                    <button
                      onClick={() => handleSendMessage(inputText)}
                      className="p-2.5 bg-[#050B14] hover:bg-[var(--color-amber-500)] text-white rounded-xl transition-colors shadow-sm"
          ...
          <AnimatePresence>
            {showToast && !isOpen && (
              <motion.div
                ...
                className="max-w-[240px] rounded-2xl px-4 py-3 text-sm shadow-xl cursor-pointer hover:shadow-2xl transition-shadow bg-white border border-slate-200/60"
              >
                <p className="font-bold text-xs mb-0.5 text-slate-800 flex items-center gap-1.5">
                  <Bot className="w-3.5 h-3.5 text-[var(--color-amber-500)]" />
                ...
                <div aria-hidden="true" className="absolute -bottom-2 right-[26px] w-4 h-2 overflow-hidden">
                  <div className="w-3 h-3 rotate-45 translate-x-0.5 -translate-y-1.5 border-r border-b bg-white border-slate-200" />
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {!isOpen && (
            <motion.button
              ...
              style={{
                ...
                boxShadow: "0 8px 32px rgba(5, 11, 20, 0.3)",
                border: "2px solid rgba(232, 96, 10, 0.3)",
              }}
              aria-label="Open AI Advisor Chat"
            >
              {/* Outer pulsing ring */}
              <span className="absolute inset-0 rounded-full border border-[var(--color-amber-500)]/45 animate-ping opacity-75" />
              
              <MessageSquare className="w-6 h-6 text-white" />
              
              {/* Mini brand indicator logo */}
              <div className="absolute -top-1 -right-1 bg-[#25D366] p-1 rounded-full border-2 border-[#050B14]">
